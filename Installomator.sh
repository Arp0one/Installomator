...
set_defaults() {
    ...
    installomatorVersion="Custom" # MDM Enabled - Use: Release|Main|Custom

    installomatorVersionCustomRepoPath="Arp0one/Installomator" # MDM Enabled

    installomatorVersionCustomBranchName="custom" # MDM Enabled
    ...
    supportTeamWebsite="" # MDM Enabled
    ...
    DISPLAY_STRING_FORMAT_DATE="%a %d %b" # Exemple français : dim. 26 juil.
    readonly DISPLAY_STRING_FORMAT_DATE
    
    # Deadline time display format.
    DISPLAY_STRING_FORMAT_TIME="+%H:%M" # Format français sur 24 heures.
...
set_display_strings_language() {
    ...
    display_string_defer_today_button="Reporter"
    display_string_defer_today_button_test="Reporter"
    
    #### Language for the defer button in dialogs when the deferral time is tomorrow.
    display_string_defer_tomorrow_button="Reporter jusqu’à demain"
    
    #### Language for the defer button in dialogs when the deferral time is in the future.
    display_string_defer_future_button="Reporter jusqu’au"
    
    ### Language for various deferral timer durations.
    display_string_minutes="minutes"
    display_string_hour="heure"
    display_string_hours="heures"
    display_string_and="et"
    display_string_days="jours"
    display_string_times="fois"
    display_string_there_are="Les"
    
    #### Language for the App Discovery dialog
    display_string_discovery_message="Analyse des applications installées"
    display_string_discovery_action_message="Analyse en cours"
    display_string_discovery_progress="Recherche en cours"

    #### Language for the Staging / Background Patch Closed Apps dialog (InteractiveMode 2 only)
    display_string_staging_message="Préparation des mises à jour"
    display_string_staging_progress="Préparation en cours"
    display_string_silent_patch_progress="Installation des mises à jour pour les applications fermées"
    
    #### Language for the Deferral Dialog with Deferrals
    display_string_deferral_button1="Reporter"
    display_string_deferral_button2="Installer maintenant"
    display_string_deferral_infobox1="Report possible jusqu’au"
    display_string_deferral_infobox2="sur"
    display_string_deferral_infobox3="reports restants\n"
    display_string_deferral_message_01="Vous pouvez choisir **Installer maintenant** pour fermer les applications et appliquer les mises à jour, ou **Reporter** pour les différer."
    display_string_deferral_message_02="**application(s)** nécessitent des mises à jour. \n\n"
    display_string_deferral_unlimited="Aucune date limite et nombre de reports illimité\n"
    display_string_deferral_selecttitle="Reporter les MàJ de :"

    #### Language for the Current/New Version subtitles shown under each app in the
    #### deferral and hard deadline dialog patch lists
    display_string_version_current="Version actuelle :"
    display_string_version_new="Nouvelle version :"

    #### Language for the Install Now confirmation mini dialog
    display_string_confirminstall_message="Voulez-vous vraiment installer les mises à jour maintenant ? Les applications indiquées seront fermées."
    display_string_confirminstall_button1="Oui, installer maintenant"
    display_string_confirminstall_button2="Non, revenir"
    display_string_confirminstall_countdown="Poursuite automatique dans"
    display_string_confirminstall_countdown_suffix="secondes…"
    
    #### Language for the Deferral Dialog with NO deferrals remaining
    display_string_deferraldeadline_button1="Installer maintenant"
    display_string_deferraldeadline_button2="Aucun report restant"
    display_string_deferraldeadline_infobox="Les mises à jour seront installées automatiquement à la fin du délai. \n\n #### Aucun report restant ####"
    display_string_deferraldeadline_message_deadline="application(s) nécessitent une mise à jour.\n\n Vous avez atteint la limite maximale de report de"
    
    #### Language for the Patching dialog
    display_string_patching_button1="Terminé"
    display_string_patching_checking="Vérification en cours"
    display_string_patching_progress="Traitement en cours"
    display_string_patching_infobox_computer_name="**Nom de l’ordinateur :**"
    display_string_patching_infobox_macos_version="**Version de macOS :**"
    display_string_patching_infobox_updates="Mises à jour :"
    display_string_patching_message="Mise à jour des applications suivantes"
    
    #### Language for Patching Complete Dialog
    display_string_complete_progress="Mises à jour terminées !"
    display_string_complete_errors="Certaines mises à jour n’ont pas été effectuées."
    
    #### All apps up to date
    display_string_uptodate_button1="Fermer"
    display_string_uptodate_message="Toutes les applications sont à jour."
    
    #### Help Message
    display_string_help_message_intro="Si vous avez besoin d’aide, veuillez contacter"
    display_string_help_message_telephone="Téléphone"
    display_string_help_message_email="E-mail"
    display_string_help_message_help_website="Site web d’assistance"
    display_string_help_message_computer_info="Informations sur l’ordinateur"
    display_string_help_message_operating_system="macOS"
    display_string_help_message_serial="Numéro de série"
    display_string_help_message_dialog="swiftDialog"
    display_string_help_message_started="Démarrage d’AAP"
    display_string_help_message_script_version="App Auto-Patch"
    display_string_help_message_software_version="Version du logiciel"
...
workflow_startup() {
    ...
    # Use the App Auto-Patch logo unless a managed custom icon was provided.
    if [[ ! -n "$dialog_icon_option" ]]; then
        if [[ -f "${appAutoPatchFolder}/AAPLogo.png" ]]; then
            log_info "No custom dialog icon configured, using the App Auto-Patch logo."
            icon="${appAutoPatchFolder}/AAPLogo.png"
        elif system_profiler SPPowerDataType | grep -q "Battery Power"; then
            log_warning "App Auto-Patch logo not found, using the laptop SF symbol instead."
            icon="SF=laptopcomputer.and.arrow.down,weight=regular,palette=gray,red"
        else
            log_warning "App Auto-Patch logo not found, using the desktop SF symbol instead."
...
check_deadlines_days_date() {
    ...
    if [[ -n "${deadline_days_epoch}" ]]; then
        ...
        display_string_deadline_days_only_date=$(env LANG=fr_FR.UTF-8 LC_ALL=fr_FR.UTF-8 date -r "${deadline_days_epoch}" "+${DISPLAY_STRING_FORMAT_DATE}")
...
set_auto_launch_deferral() {
    ...
    local deferral_weekday
    deferral_weekday=$(date -r "${deferral_timestamp}" +%u)
    if [[ "${deferral_weekday}" == "6" ]]; then
        deferral_timestamp=$((deferral_timestamp + 2 * 86400))
        log_notice "Deferral would end on Saturday; moving it to Monday at the same time."
    elif [[ "${deferral_weekday}" == "7" ]]; then
        deferral_timestamp=$((deferral_timestamp + 86400))
        log_notice "Deferral would end on Sunday; moving it to Monday at the same time."
    fi
...
swiftDialogCompleteDialogPatching(){
    ...
    if [ ${InteractiveModeOption} -ge 1 ]; then
        if [[ ${errorCount:-0} -gt 0 ]]; then
            swiftDialogUpdate "progresstext: ${display_string_complete_errors}"
        else
...
set_deferral_menu() {
    ...
    if [[ -n "${deferral_timer_menu_minutes}" ]]; then
        ...
        for array_index in {1..${#deferral_timer_menu_minutes_array[@]}}; do
            ...
            # Display concise durations; the weekend adjustment is applied when scheduling.
            if [[ $minutes -lt 60 ]]; then
                deferral_timer_menu_display_array+=("${minutes} ${variable_display_string_minutes}")
            elif [[ $minutes -eq 60 ]]; then
                deferral_timer_menu_display_array+=("1 ${variable_display_string_hour}")
            elif [[ $minutes -gt 60 && $minutes -lt 1440 ]]; then
                hours=$((minutes / 60))
                remaining_minutes=$((minutes % 60))
                if [[ $remaining_minutes -eq 0 ]]; then
                    deferral_timer_menu_display_concat="${hours} ${variable_display_string_hours}"
                    deferral_timer_menu_display_array+=("${deferral_timer_menu_display_concat}")
                else
                    deferral_timer_menu_display_concat="${hours} ${variable_display_string_hours} ${variable_display_string_and} ${remaining_minutes} ${variable_display_string_minutes}"
                    deferral_timer_menu_display_array+=("${deferral_timer_menu_display_concat}")
                fi
            else
                days=$((minutes / 1440))
                remaining_minutes=$((minutes % 1440))
                if [[ $remaining_minutes -eq 0 && $days -eq 1 ]]; then
                    deferral_timer_menu_display_array+=("1 jour")
                elif [[ $remaining_minutes -eq 0 ]]; then
                    deferral_timer_menu_display_array+=("${days} jours")
                else
                    deferral_timer_menu_display_array+=("${minutes} ${variable_display_string_minutes}")
                fi
            fi
...
dialog_install_or_defer() {
    ...
    while true; do
        ...
        if [[ -n "${deferral_timer_menu_minutes}" ]]; then
            ...
            local select_default_index
            for select_default_index in {1..${#deferral_timer_menu_minutes_array[@]}}; do
                if [[ "${deferral_timer_menu_minutes_array[${select_default_index}]}" == "${deferral_timer_minutes}" ]]; then
                    selectDefault=${deferral_timer_menu_display_array[${select_default_index}]}
                    break
                fi
            done
    ...
    for label in $queuedLabelsArray; do
        ...
        if [[ ${zoom_call_active_check_option} == "TRUE" && ${label} == "zoom"* ]] ; then
	        ...
	        if [[ -n "$CPTHOSTPID" || -n "$AOMHOSTPID" ]]; then
                ...
                if [[ $installomatorExitCode -eq 10 ]]; then
                    log_notice "User deferred ${label}; keeping it pending."
                    swiftDialogUpdate "listitem: index: $i, status: wait, statustext: Mise à jour reportée"
                    let errorCount++
                elif [ $installomatorExitCode != 0 ]; then
                    log_error "Error installing ${label}. Exit code $installomatorExitCode"
                    swiftDialogUpdate "listitem: index: $i, status: fail, statustext: Échec"
                    let errorCount++
                fi
